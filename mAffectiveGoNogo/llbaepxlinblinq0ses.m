function [l,dl,dsurr] = llbaepxlinblinqses(x,D,mu,nui,doprior,options);
% 
    ...
    for t=1:size(a,2) % this is setting t as one row and however
        ...
        if ~isnan(s(t)) & ~isnan(a(t)) % if the index of t within s is not
            ...
            %dl(6) = dl(6) + g*(p0(a(t))*(ses-1)*epsilon*V(s(t)) * ((a(t)==1)-p0(1))) / pg(a(t));
				...
				dQdq0(a(t),s(t)) = (1-alfa)*dQdq0(a(t),s(t));

        		tmp = dQdql(:,s(t)); 
        		dl(8) = dl(8) + g*(p0(a(t)) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
				dQdql(a(t),s(t)) = (1-alfa)*dQdql(a(t),s(t));
         end
