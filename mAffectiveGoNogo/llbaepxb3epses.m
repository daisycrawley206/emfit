...
for t=1:size(a,2) % this is setting t as one row and however
	...
	if ~isnan(s(t)) & ~isnan(a(t)) % if the index of t within s is not
		...
		sesdep = (w(t)>=2)*x(6) + (w(t)>=3)*x(7); %% question: do we use ses-1 because 
		...
		if dodiff
			...
			dl(6) = dl(6) + g*(p0(a(t))*(w(t)>=2)*epsilon*V(s(t)) * ((a(t)==1)-p0(1))) / pg(a(t));
			dl(7) = dl(7) + g*(p0(a(t))*(w(t)>=3)*epsilon*V(s(t)) * ((a(t)==1)-p0(1))) / pg(a(t));
		end
