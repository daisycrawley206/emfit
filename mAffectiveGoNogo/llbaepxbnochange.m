function [l,dl,dsurr] = llbaepxbnochange(x,D,mu,nui,doprior,options);
% 
    ...
    for t=1:size(a,2) % this is setting t as one row and however
        ...
        if t>1
            if (w(t)~=w(t-1)) | mod(t-1,60)==0; % this resets the Q values per session
                V=zeros(1,4); % we still do this in the no change model as the stimuli change
                Q=zeros(2,4); % so learning is 'new' but in this model not affected by BA
        ...
        if ~isnan(s(t)) & ~isnan(a(t)) % if the index of t within s is not
            ...
            %sesdep = (ses-1)*x(6); %% leave this in but we don't update the parameter
